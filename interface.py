...
with gr.Blocks() as iface:
    ...
    with gr.Column():
        ...
        def initialize_example_data(input_values, output_values):
            input_values = input_values.value  # Extract values from gr.State
            output_values = output_values.value  # Extract values from gr.State
            headers = input_values + output_values
            return headers

        headers = initialize_example_data(input_values, output_values)

        example_data = gr.Dataframe(
            headers=headers,  # Use the initialized headers
            datatype=["str"] * len(headers),  # Use the length of the headers
            interactive=True,
            row_count=1,
    ...
    def show_dataframe(input_values, output_values):
        input_values = input_values.value  # Extract values from gr.State
        output_values = output_values.value  # Extract values from gr.State
        data = {f"input-{i}": value for i, value in enumerate(input_values)}
        data.update({f"output-{i}": value for i, value in enumerate(output_values)})
    ...
    enter_manually_btn.click(
        show_dataframe,
        inputs=[input_values, output_values],
        outputs=[example_data, export_csv_btn, compile_button]
    )

    def process_csv(file, input_values, output_values):
        input_values = input_values.value  # Extract values from gr.State
        output_values = output_values.value  # Extract values from gr.State
        if file is not None:
            try:
                ...
                input_fields = [arg for arg in input_values if arg and arg.strip()]
                output_fields = [arg for arg in output_values if arg and arg.strip()]
    ...
    upload_csv_btn.upload(
        process_csv,
        inputs=[upload_csv_btn, input_values, output_values],  # Pass the State objects directly
    ...
    def compile(input_values, output_values, llm_model, teacher_model, dspy_module, example_data, optimizer, instructions):
        input_fields = [field for field in input_values if field.strip()]
        output_fields = [field for field in output_values if field.strip()]
        ...
        details = {
            ...
            "example_data": example_data.to_dict(orient='records'),
