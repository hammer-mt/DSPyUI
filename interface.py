    ...
    with gr.Tabs():
        with gr.TabItem("Compile Program"):
            ...
            def render_variables(input_values, output_values, file_data):
                ...
                with gr.Row():
                    ...
                    def compile(data):
                        ...
                        # Format cost display
                        cost_str = f"${cost_data.get('actual_cost_usd', 0):.4f}"
                        tokens_str = f"{cost_data.get('total_tokens', 0):,} ({cost_data.get('input_tokens', 0):,} in + {cost_data.get('output_tokens', 0):,} out)"

                        return signature, evaluation_score, optimized_prompt, gr.update(choices=row_choice_options, visible=True, value="Row 1"), gr.update(visible=True), row_choice_options, gr.update(visible=True), gr.update(visible=True), human_readable_id, gr.update(visible=True), baseline_score, cost_str, tokens_str
                ...
                compile_button.click(
                    ...
                    outputs=[signature, evaluation_score, optimized_prompt, row_selector, random_row_button, row_choice_options, generate_button, generate_output, human_readable_id, human_readable_id, baseline_score, cost_display, tokens_display]
                )
            ...
            with gr.Column() as compilation_results:
                ...
                with gr.Row():
                    cost_display = gr.Textbox(label="💰 Cost", interactive=False, info="Actual cost of compilation in USD")
                    tokens_display = gr.Textbox(label="🎫 Tokens Used", interactive=False, info="Total tokens (input + output)")
